...
cat("============================================================================\n")
cat("TASK 2 - VERSION 2: BACKWARD ELIMINATION\n")
...
# MODEL #2: BACKWARD ELIMINATION (VERSION-SPECIFIC)
cat("MODEL #2: Best ≤4 predictor model - BACKWARD ELIMINATION\n")
cat("Method: Start with all 7 predictors, remove least significant iteratively\n\n")
...
cat("Step 0: Full model (7 predictors), AIC =", round(AIC(full_model), 2), "\n")
...
while (length(backward_preds) > 4) {
  ...
  backward_model <- lm(as.formula(paste("Overall ~", paste(backward_preds, collapse = " + "))), data = data)
  cat(sprintf("Step %d: Removed %s, AIC = %.2f\n", step_num, worst_pred, AIC(backward_model)))
  step_num <- step_num + 1
}

model2 <- backward_model
selected <- backward_preds
...
# Simple forward selection with Crime mandatory
london_sel <- c("Crime")
nl_sel <- c("Crime")
...
cat("\n============================================================================\n")
cat("VERSION 2 COMPLETE - BACKWARD ELIMINATION EMPHASIS\n")
cat("============================================================================\n")
